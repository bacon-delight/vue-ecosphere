<template>
	<div class="wrapper">
		<div class="wrapper__title">{{ $t("pages.data_entry.search_dropdown.title") }}</div>
		<div class="wrapper__body">
			<div class="section">
				<div class="section__flex">
					<SearchDropdown :label="inputAttributeAlterations.includes('label') ? 'Select your favourite fruit' : null" :placeholder="inputAttributeAlterations.includes('placeholder') ? 'Search & Select Option' : null" :assistive-text="inputAttributeAlterations.includes('assistiveText') ? 'Firethorns are inedible, but they are good source for birds!' : null" :options="options" :state="inputStateAlteration" :allow-clear="inputAttributeAlterations.includes('allowClear')" :disabled="inputAttributeAlterations.includes('disabled')" :outline="inputAttributeAlterations.includes('outline')" :alert-message="inputAttributeAlterations.includes('alertMessage') ? 'Alert Message' : null" :default="inputAttributeAlterations.includes('default') ? 2 : null" :hue="inputHueAlteration" expand="below"></SearchDropdown>
				</div>
			</div>
			<div class="section">
			...
			<MarkdownParser :content="$t('pages.data_entry.search_dropdown.content')"></MarkdownParser>
		</div>
	</div>
</template>
